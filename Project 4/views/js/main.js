...
//NO NEED FOR THE ID FOR PIZZA ELEMENT
var pizzaElementGenerator = function() {
  ...
  //THE PIZZA ID ISN'T USED ANYWHERE. REMOVING
...
var resizePizzas = function(size) {
  ...
  // MOVING THE ELEMENT OUT OF THE LOOP
  // ASSIGNING THE LENGTH TO A VARIABLE INSTEAD OF CALCULATING EVERY TIME THE CHECK IS REACHED
  // CLEANED UP THE SLIDER RESIZING CODE
  var windowwidth = document.querySelector("#randomPizzas").offsetWidth; //GLOBAL VARIABLE
  function changePizzaSizes(size) {

    var dx = 0,
    randomPizzaContainer = document.querySelectorAll(".randomPizzaContainer"),
        oldwidth = randomPizzaContainer[0].offsetWidth,//EVERY ELEMENT HAS SIMILAR WIDTH AND HENCE NO NEED TO CALCULATE FOR ALL
    ...
    switch(size) {
      ...
      var newwidth = (oldwidth + dx) + 'px'; //REUSE OLDWIDTH
      $('.randomPizzaContainer').width(newwidth); //SETTING WIDTH FOR ALL ELEMENTS INSTEAD OF A LOOP
  }
...
var pizzasDiv = document.getElementById("randomPizzas"); //MOVING THIS VARIABLE OUT OF THE LOOP
for (var i = 2; i < 100; i++) {
...
function updatePositions() {
  ...
  var scrollTop = document.body.scrollTop / 1250; //MOVED THE ACCESS TO DOCUMENT OUT
  var length = items.length; //MOVED THE LENGTH CALCULATION OUT OF THE LOOP AND TO A VARIABLE
  for (var i = 0; i < length; i++) {
    // var phase = Math.sin(scrollTop + (i % 5));
    items[i].style.transform = "translateX(" + 100 * Math.sin(scrollTop + (i % 5)) + "px)"; //USING TRANSFORM INSTEAD
  }
...
var movingSlider = document.querySelector("#movingPizzas1"); //MOVING THIS OUT TO AVOID MULTIPLE QUERIES
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8,
   s = 256;
   //REDUCING THE NUMBER OF PIZZAS AS NOT ALL OF THEM WILL BE DISPLAYED ANY WAYS. cols = 8 IMPLIES 8 COLUMNS. THE MOBILE SHOWS 4 ROWS. A LARGE
   //MONITOR MIGHT SHOW 6? HENCE 48 PIZZAS.
  for (var i = 0; i < 48; i++) {
    ...
    elem.style.left = elem.basicLeft + 'px'; //INITIAL LOCATION
    movingSlider.appendChild(elem);
  }
  updatePositions();
});
